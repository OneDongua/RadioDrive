...
object StationPopupMenu {
    fun open(
        ...
        val gravity =
            if (view.y + view.height > view.rootView.height / 2) Gravity.TOP else Gravity.BOTTOM
        val popupMenu = popupMenu {
            dropdownGravity = gravity
            style =
                if (Utils.isDarkTheme(context)) R.style.Widget_MPM_Menu_Dark else R.style.Widget_MPM_Menu
            section {
                if (play_external) {
                    item {
                    ...
                    item {
                        ...
                        iconDrawable = IconicsDrawable(
                            context,
                            CommunityMaterial.Icon2.cmd_play_box_outline
                        ).sizeDp(24)
                        callback = {
                            Utils.playAndWarnIfMetered(
                                context.applicationContext as RadioDroidApp, station,
                                PlayerType.EXTERNAL
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1) {
                    item {
                        ...
                        callback = {
                            station.prepareShortcut(
                                context,
                                itemAdapterStation.CreatePinShortcutListener()
                            )
                        }
                    }
                }
